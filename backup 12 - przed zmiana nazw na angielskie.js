...
// global
var ciag_liczb = '';    //  we will operate on this string
//const znak_dzialania = ['^', '*', '/', '+', '-'];   //  table with action signs | redundant :c
...
function kwadrat(){
    ...
    let kwadrat = Math.pow(parseFloat( nr[0] ),  2);
    console.log(kwadrat);
    zmien_ostatni_wyraz(nr[1], kwadrat);
}
...
function pierwiastek() {
    ...
    if( parseFloat(nr[0]) >= 0){
        liczba = parseFloat(nr[0]);
    }else{
        liczba = parseFloat(nr[0]) * (-1);
    }
    let pierwiastek = Math.sqrt( liczba ); // pierwiastek z liczby
    console.log(nr[0] + " " + pierwiastek);
    zmien_ostatni_wyraz(nr[1], pierwiastek);  
}

function odwrotnosc() {
    let nr = wyznacz_ostatni_wyraz(ciag_liczb);
    let l_odwrotna = 1 / parseFloat( nr[0] );
    //console.log( l_odwrotna +" l_odwrotna");
    zmien_ostatni_wyraz(nr[1], l_odwrotna);
}
...
function ce() {
    ...
    if(ciag_liczb == '-'){
        ...
        //  #0
        ciag_liczb = Array.from(ciag_liczb);// zmiana w tablice
        //  #1
        
        ciag_liczb.splice( nr[1], ciag_liczb.length ); // usuwa koncówke
        console.log( ciag_liczb +" bez koncówki");
        ciag_liczb = ciag_liczb.join("");//zmian atablicy w stringa
...
function plusminus() {
    ...
    if(ciag_liczb == '-'){
        ...
        //  #0
        zmien_ostatni_wyraz(nr[1], zmiana);
    }
    wypisz_ciag_liczb();
}
...
function zmien_ostatni_wyraz(nr, l_concat){
    ciag_liczb = Array.from(ciag_liczb); // zmiana w tablice
    ciag_liczb.splice( nr, ciag_liczb.length ); // usuwa koncówke
    //console.log( ciag_liczb +" bez koncówki");
    ciag_liczb = ciag_liczb.concat(l_concat); // dodaje zmieniona liczbe na koncu
